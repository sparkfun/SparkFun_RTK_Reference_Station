/*
  ...
  Use two jumper wires to connect:
  RXD (16) <- GNSS TX2
  TXD (17) -> GNSS RX2
...
void setup()
{
  ...
  Serial1.begin(38400, SERIAL_8N1, SERIAL1_RX, SERIAL1_TX);

  while (!theGNSS.begin(Serial1)) // Start the GNSS on Serial1
  {
    Serial.println("GNSS not detected. Retrying...");
  }

  theGNSS.setUART2Output(COM_TYPE_UBX); //Set the UART2 port to output UBX only (turn off NMEA noise)
...
void loop()
{
  ...
  if (theGNSS.getHPPOSLLH()) // Returns true when fresh data is available
  {
    ...
    Serial.print(", Altitude MSL (m): ");
    Serial.print(f_MSL, 4); // Print the altitude with 4 decimal places
